...
CREATE OR REPLACE PACKAGE BODY pkg_service AS
    ...
    PROCEDURE show_expired_cars IS
        CURSOR expired_cars_cursor IS
            SELECT v.license_plate,
                   ...
                   i.remarks
              FROM vehicles v
              JOIN customers c ON v.owner_id = c.customerid
              JOIN inspections i ON v.vehicleid = i.vehicleid
             WHERE i.expiry_date < SYSDATE;

        expired_car_record expired_cars_cursor%ROWTYPE;
    BEGIN
        DBMS_OUTPUT.PUT_LINE('Lejárt műszakival rendelkezõ autók:');
    ...
    BEGIN
        FOR inspection_record IN inspections_by_month LOOP
            DBMS_OUTPUT.PUT_LINE(inspection_record.MONTH || ' | ' || inspection_record.total_inspections);
            EXIT; -- Csak a legtöbb vizsgával rendelkező hónapot mutatja
        END LOOP;
    END top_inspection_month;
...
--Adatok beszúrása
CREATE OR REPLACE PROCEDURE data_insert AS
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('Új ügyfél hozzáadva: ' || v_name || ' (ID: ' || v_customerID || ')');
    ...
    DBMS_OUTPUT.PUT_LINE('Új jármű hozzáadva: ' || v_license_plate || ' (ID: ' || v_vehicleID || ')');
    ...
    DBMS_OUTPUT.PUT_LINE('Új műszaki vizsga hozzáadva: ' || v_inspectionID);
